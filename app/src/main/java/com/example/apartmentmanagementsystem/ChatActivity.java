...
public class ChatActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        android.util.Log.d("CHAT_AUTH", "ID: " + currentUserId + " | Token: " + accessToken);
    ...
    private void initViews() {
        ...
        if (currentUserName != null && !currentUserName.isEmpty() && tvHeaderAvatar != null) {
            tvHeaderAvatar.setText(String.valueOf(currentUserName.charAt(0)).toUpperCase());
        }
        if (backButton != null) {
            backButton.setOnClickListener(v -> {
                Intent intent = new Intent(ChatActivity.this, FeedActivity.class); // මෙතනට ඔබේ Feed screen එකේ නම දාන්න
    ...
    private String getInitials(String name) {
        if (name == null || name.trim().isEmpty() || name.equalsIgnoreCase("Resident")) {
            return "ME";
        }
        ...
        if (words.length > 0 && !words[0].isEmpty()) {
            initials.append(words[0].toUpperCase().charAt(0));
        }

        if (words.length > 1 && !words[1].isEmpty()) {
            initials.append(words[1].toUpperCase().charAt(0));
        }

        return initials.length() > 0 ? initials.toString() : "ME";
    }
    ...
    private void loadMessages() {
        new Thread(() -> {
            try {
                String msgUrl = SupabaseClient.SUPABASE_URL + "/rest/v1/messages?select=id,user_id,content,reply_to_id,created_at&order=created_at.asc&limit=100";
                ...
                if (msgArr.length() > 0) {
                    ...
                    try {
                        String usersUrl = SupabaseClient.SUPABASE_URL + "/rest/v1/users?select=id,full_name&id=in.(" + idList + ")";
                        ...
                        for (int i = 0; i < usersArr.length(); i++) {
                            ...
                            nameMap.put(u.getString("id"), u.optString("full_name", "Resident"));
                        }
                ...
                for (int i = 0; i < msgArr.length(); i++) {
                    ...
                    if (locallyReactedIds != null && locallyReactedIds.contains(msgId)) {
    ...
    private void sendMessage() {
        ...
        new Thread(() -> {
            try {
                HttpURLConnection conn = (HttpURLConnection) new URL(SupabaseClient.SUPABASE_URL + "/rest/v1/messages").openConnection();
    ...
    public void onLongPress(Message message, boolean isOwn) {
        if (isOwn) {
            new AlertDialog.Builder(this)
                    .setTitle("Options")
                    .setItems(new String[]{"Delete Message"}, (d, w) -> deleteMessage(message))
                    .show();
        }
    }
    ...
    private void deleteMessage(Message message) {
        new Thread(() -> {
            try {
                String url = SupabaseClient.SUPABASE_URL + "/rest/v1/messages?id=eq." + message.getId();
    ...
    @Override
    public void onReact(Message message) {

        if (!locallyReactedIds.contains(message.getId())) {
            locallyReactedIds.add(message.getId());
        }
    }
